...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        def get_counts(filenames:list):
            ...
            for file_ in filenames:
                for word in self.word_set(file_):
                    if not word in my_d:
                        my_d[word] = 1
                    else:
                        my_d[word] += 1
    ...
    def predict(self, filename:str):
        ...
        prob_ham = []
        prob_spam = []
        probs_overall_ham = self.num_train_hams/(self.num_train_spams + self.num_train_hams)
        probs_overall_spam = 1 - probs_overall_ham
        spam_deno = self.num_train_spams + 2
        ham_deno = self.num_train_hams + 2
        for word in self.word_set(filename): # ham
            prob_ham.append((self.word_counts_ham.get(word, 0) + 1)/ham_deno)
            prob_spam.append((self.word_counts_spam.get(word, 0) + 1)/spam_deno)
        val = np.log(probs_overall_spam) + sum(map(lambda x: np.log(x), prob_spam))
        val2 =  np.log(probs_overall_ham) + sum(map(lambda x: np.log(x), prob_ham))
        if val2 >= val:
