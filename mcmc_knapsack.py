...
class KnapsackMcmc:
    ...
    def value(self, subset):
        ...
        return np.dot(subset, self.values) # TODO: Your code here (~1-4 lines)
    ...
    def weight(self, subset):
        ...
        return np.dot(subset, self.weights) # TODO: Your code here (~1-4 lines)
    ...
    def mcmc(self, W:float, T:float=0, num_iter:int=5000):
        ...
        :return: A tuple of three elements:
            ...
            CURRENT subset at the beginning of each iteration. This is NOT the current 
        ...
        Notes:
        1. Use the pseudocode sampled from open source.
        ...
        best_subset = np.zeros(self.num_items)
        ret_list = []
        for i in range(1, num_iter + 1):
            gen_num = np.random.randint(0, self.num_items)
            new_subset = np.copy(subset)
            if new_subset[gen_num] == 1:
                new_subset[gen_num] = 0
            else:
                new_subset[gen_num] = 1
            net_change = self.value(new_subset) - self.value(subset)
            ret_list.append(self.value(subset))
            if self.weight(new_subset) <= W:
                if net_change > 0 or (T > 0 and np.random.rand() < np.exp(net_change/T)):
                    subset = new_subset
            if self.value(subset) > self.value(best_subset):
                best_subset = subset
        return best_subset, self.value(best_subset), ret_list
